...
class Players:
	...
	#Skins and Pins and StarPowers
	UnlockedPins = {}
	for x in emotes_id:
		UnlockedPins.update({str(x): 0})
		
	UnlockedSkins = {}
	for x in skins_id:
		UnlockedSkins.update({str(x): 0})
		
	StarPowerUnlocked = {}
	for x in card_skills_id:
		StarPowerUnlocked.update({str(x): 0})

	name = "Guest"
	tutorial = settings['TutorialState']
	trophy_road = 200
	player_experience = 100000
	collected_experience = 100000
	profile_icon = 0
	name_color = 0
	do_not_distrub = 0
